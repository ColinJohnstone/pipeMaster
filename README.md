...
- **Photo import (OMR)** — take or upload a photo of printed pipe music; a client-side computer-vision pipeline (deskew → staff detection → distance-transform notehead detection → gracenote clustering → **embellishment reverse-matching** against the registry) reads pitches, **embellishments**, and dots into an editable draft. Note lengths default to quavers for you to tidy.
