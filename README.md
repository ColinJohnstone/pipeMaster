...
- **Photo / PDF import (OMR)** — take or upload a photo, or a **PDF page** (rasterised with pdf.js), of printed pipe music; a client-side computer-vision pipeline (deskew → staff detection → distance-transform notehead detection → stem/beam/flag analysis → gracenote clustering → **embellishment reverse-matching**) reads pitches, note lengths, embellishments, and dots. Then a **correction step** lets you tap to add/remove noteheads and nudge the detection **sensitivity** before importing — and your corrections are saved as a **labelled dataset** (downloadable as JSON) to improve recognition over time.
